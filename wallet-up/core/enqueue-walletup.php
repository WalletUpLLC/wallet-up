...
class wallet_up {
	...
	function wallet_up_page_init() {
		...
		add_settings_field(
			'paypal_holder', // id
			'Type your PayPal.me username (Example: <a href="https://www.paypal.com/paypalme/WalletUp/1" target="_blank">WalletUp</a>)', // title
		...
		add_settings_field(
			'cashapp_holder', // id
			'Type your $cashtag (Example: <a href="https://cash.app/WalletUpLLC/1" target="_blank">$WalletUpLLC</a>)', // title
		...
		add_settings_field(
			'venmo_holder', // id
			'Type your venmo username (Example:<a href="https://venmo.com/WalletUp?txn=pay&amount=1&note=Demo%20Only" target="_blank">WalletUp</a>)', // title
	...
	function wallet_up_holder_no_callback() {
		...
		function wallet_up_paypal_holder_callback() {
			...
			if ( isset( $wallet_up_transfer[ 'paypal_holder' ] ) ) {
				$walup_try_it = '<a class="link-primary" href="https://www.paypal.com/paypalme/' . $wallet_up_transfer[ 'paypal_holder' ] . "/" . 1 . '" target="_blank"><br><form><input type="button" value="Click here to try it"></form></a>';
			} else {
				$walup_try_it = null;
			}
			printf(
	...
	function wallet_up_cashapp_holder_callback() {
		...
		if ( isset( $wallet_up_transfer[ 'cashapp_holder' ] ) ) {
			$walup_try_it = '<a class="link-primary" href="https://cash.app/' . $wallet_up_transfer[ 'cashapp_holder' ] . "/" . 1 . '" target="_blank"><br><form><input type="button" value="Click here to try it"></form></a>';
		} else {
			$walup_try_it = null;
		}
		printf(
	...
	function wallet_up_venmo_holder_callback() {
		...
		if ( isset( $wallet_up_transfer[ 'venmo_holder' ] ) ) {
			$walup_try_it = '<a class="link-primary" href="https://venmo.com/' . esc_attr( wp_kses_post( $wallet_up_transfer[ 'venmo_holder' ] ) ) . '?txn=pay&amount=1&note=Wallet Up Demo" target="_blank"><br><form><input type="button" value="Click here to try it"></form></a>';
		} else {
			$walup_try_it = null;
		}
		printf(
